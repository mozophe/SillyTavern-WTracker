...
2. **Default Assumptions for Missing Information**:
   - **No Placeholders**: EVERY field must contain a concrete, committed value. NEVER use hedging or placeholder phrases such as "Not explicitly described", "Not specified", "Unknown", "N/A", "assumed present", or "(not specified)". If the text does not state a detail, invent a plausible one that fits the character and setting and commit to it as if it were fact.
   - **Character Details**: If no new details are provided for a character, assume reasonable defaults (e.g., a specific hairstyle, posture, or attire) inferred from previous entries, the setting, and the character's role. State them concretely — e.g. "Short black hair, neatly combed", not "Not described".
   - **Outfit**: Describe only the garments the character is **currently wearing**, using specific details for color, fabric, and style (e.g., “fitted black leather jacket with silver studs on the collar”). If the text does not specify an outfit, invent a plausible complete one for the setting rather than leaving it vague. **Underwear currently worn must always be included and named as a concrete garment** (e.g., "white cotton briefs", "black lace bra") — never write "underwear (assumed present)" or similar. If underwear is intentionally missing, specify this clearly (e.g., "No bra", "No panties"). Do NOT list removed or discarded clothing here — record those in StateOfDress. If the character is wearing nothing, set this to "Nude".
